export default {
    categories: {
        title: 'Τι θα φάμε σήμερα;',
        cards: [
            {
                name: 'Pizza',
                link: 'shops/pizza',
                bgImg: 'https://florinafood.gr/imgs/menu/pizza.jpg'
            },
            {
                name: 'Γυρος',
                link: 'shops/guros',
                bgImg: 'https://florinafood.gr/imgs/menu/souvlakia.jpg'
            },
            {
                name: 'Burger',
                link: 'shops/burger',
                bgImg: 'https://florinafood.gr/imgs/menu/burger.jpg',
            },
            {
                name: 'Hot Dog',
                link: 'shops/hot-dog',
                bgImg: 'https://florinafood.gr/imgs/menu/hotdog.jpg',
            },
            {
                name: 'Sandwich',
                link: 'shops/sandwich',
                bgImg: 'https://florinafood.gr/imgs/menu/sandwich.jpg',
            },
            {
                name: 'Κρεπες',
                link: 'shops/crepes',
                bgImg: 'https://florinafood.gr/imgs/menu/crepe.jpg',
            },
        ],
    },
    coffee: {
        title: 'Παράγγειλε καφεδάκι από ένα κατάστημα μας!',
        text: 'Τώρα μπορείς να βρεις, όλα τα καφεκοπτεία της πόλης και να απολαύσεις τον αγαπημένο σου καφέ απλά και εύκολα με ένα click στο florinafood.gr',
        button: {
            to: '/shops/coffee',
            label: 'Κάνε click για καταστήματα'
        }
    }
};